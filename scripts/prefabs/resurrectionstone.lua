...
local function OnHaunt(inst, haunter)
    if inst._task == nil and
        ...
        inst.AnimState:IsCurrentAnimation("idle_activate") and
        not haunter:HasTag("tank") then
        ...
        return true
    elseif haunter:HasTag("tank") then
        haunter.components.talker:Say(STRINGS.TANK_CAN_NOT_USE_RESURRECTIONSTONE)
    end
end
...
local function OnActivateResurrection(inst, guy)
    if inst._task ~= nil and not guy:HasTag("tank") then
        inst._task:Cancel()
        inst._task = nil
    end
    if not guy:HasTag("tank") then
